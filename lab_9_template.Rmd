...
# bedrooms, bathrooms and sqft are all telling us the same time. PricePerSqFt has correlation with Price. 

homes_lm1
#depedent variable is price as a function of....
...
homes_lm2
#arroyo grande is the reference level since its alphabetical 
...
multiple R-squared value does not adjust for the fact that as you increase the number of predictors, the model fit increases. adjusted R-squared does. 
...
# c. Check levels:
#use levels() will always be alphabetical by default

# d. Reassign reference level of "Status" to "Regular":
#fct_relevel*() in forcats


# e. Now run the regression again - same equation, but now the reference level is different (Status = Regular): s
...
# doesn't mean anything if its not used for comparison


# Answer: which would you pick? 
#based on AIC, it will be the first, but we know that the first one isn't the best. 
...
First, you need to create a new data frame of values that contain ALL NECESSARY VARIABLES **with the same variable names viewAND level strings**.
...
# Create a line graph with square footage on the x-axis, predicted price on y-axis, with color dependent on City and facet_wrapped by sale status (follow along): fit.fit is model fit
predict_graph <- ggplot(predict_df, aes(x= SqFt, y= fit.fit)) +
  geom_line(aes(color=City)) +
  geom_point(data=homes_sub,aes(x=SqFt, y=Price))+
  facet_wrap(~Status)+ 
  theme_bw()
